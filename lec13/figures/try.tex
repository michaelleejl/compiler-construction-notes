\begin{tikzpicture}
  ...
  \node[draw,rounded corners,anchor=south west,minimum width=10ex,minimum height=3ex] (rj)
     at (rcl.north west) {  };
  \node[circle,draw,darkest,fill=darkred!0.5!blue,opacity=0.1,text opacity=1.0,draw opacity=1.0,inner sep=0pt,minimum width=2.5ex] at (rj) { \scriptsize k };
...
\node[anchor=west,circle,draw,darkest,fill=green!30!background,opacity=0.1,text opacity=1.0,draw opacity=1.0,inner sep=0pt,minimum width=2.5ex] (epl) at ($(lstack.east) + (3ex,-1ex)$) { \scriptsize e2  };
\draw[->](epl) -- ($(lstack.east) + (0,-1ex)$);
...
\draw[->](epr) -- (rv.east);
  ...
  \draw[->] ($(lstack.north east) + (1ex,-1ex)$) -- node[midway,fill=white] { \small\texttt{TRY}$\;k$ } ($(rstack.north west) + (-1ex,-1ex)$);
