...
camlExample__set_of_closures_29:
...
Clearly, there is a huge gap (aptly named ``The Gap'') between the code in \Cref{code:fib-ocaml} and in \Cref{code:fib-assembly}.

We'll return to \emph{The Gap} in a moment, but it is the compiler's fundamental responsibility to \emph{bridge the gap} between the high level syntax in \Cref{code:fib-ocaml} --- one that is designed for humans like you and me to understand --- and the machine oriented syntax in \Cref{code:fib-assembly}. Compilers thus allow those of us who think about \emph{the programmer} --- programming language designers and human-computer interaction people that play around with fancy type systems (like those found in \texttt{Agda} or \texttt{Idris}), interesting evaluation strategies (Call By Name or Call By Value), or HCI properties --- to avoid having to think about the machine. Symmetrically, they also allow those of us who think about \emph{the machine} --- computer architects who are considering different or novel instruction set architectures --- to avoid thinking about the human. 
...
You haven't learnt \texttt{Agda} yet, but here's a basic definition for a natural numbers type in \texttt{Agda} 
...
And here's a version that fails to compile 
...
In some sense, therefore, a compiler is like that one friend who is an utter genius when it comes to some really hard things --- like training a neural network or solving category theory questions --- yet utterly rubbish at others --- like tying their shoes or brushing their teeth. By understanding them, we hope to understand \emph{how they work}, and thus why they are good at the things they are good at, and bad at the things they are bad at. As a result, no matter what you use them for, you'll hopefully be able to have a sense of \emph{why they do what they do}, and therefore \emph{use them more effectively}. If you go on to design programming languages, for example, understanding how compilers work will help you better weigh the consequences of design decisions. If you go on to become a computer architect, the vast majority of the code you'll be running won't be written directly by a person, but instead compiled by a compiler, and you'll have to figure out whether your design decisions can be exploited by a compiler effectively.
...
\textbf{Compilers \emph{process} programs.} You'll hopefully agree that programs --- and the types of programs people come up with --- are full of rich ideas. Take, for example, the Fast Inverse Square Root Algorithm (whose details you don't have to understand)

\begin{code}[The Fast Inverse Square Root Algorithm]
...
Now --- at the risk of personifying compilers too much --- this is an \emph{incredibly opaque} piece of code whose semantics is \emph{not at all clear}. But a compiler \textbf{has} to be able to process this code, and has to (in some sense) ``understand'' its semantic meaning, so it can perform a transformation that's \emph{safe}, plus some optimisations. In other words, since \emph{programs} are interesting, compilers (program \textit{processors} or \textit{transformers}) are interesting too.
...
\end{minted}
Its merits are hard to see in this trivial example, but it is used widely in compilers, and indeed crops up in other places! In {\sffamily Part II Bioinformatics}, you'll learn about \emph{the Four Russians Technique} of speeding up the Longest Common Sub-sequence (LCS) algorithm ({\sffamily Part IA Algorithms}), which (morally) relies on this very same trick. 
...
Which, as we'll see, is actually a way of implementing the \texttt{while} function! In {\sffamily Denotational Semantics}, you'll learn another way of specifying a semantics for \texttt{while}, via the \emph{Kleene-Tarski fixed-point theorem}, and indeed this is \emph{another} way of implementing \texttt{while}.
...
\begin{minipage}[t]{0.85\textwidth}    
    ...
    The first is to take the annotated AST and transform it into a re-targetable representation, or an intermediate representation. Examples of such representations include the \texttt{Java Bytecode}, the \texttt{Jargon VM} (which we'll be compiling down to in this course) and \texttt{3 Address Code} (which you'll see in {\sffamily Part II Optimising Compilers}). In a hand-wavy way, you can think of these re-targetable or intermediate representations as the intersection of the capabilities of all the different machines --- \texttt{ARM}, \texttt{x86}, and much more. 
...
int f(int x) {
   ...
   // some code which never uses y
    ...
    \textbf{Non-Example 1: Register Allocation.} There is a trivial way to allocate registers: allocate each variable to its own register, and spill (push to memory) all variables which cannot be allocated a register. This is non-optimal. More optimal strategies are based on \textit{live variable analysis} and \textit{graph colouring} (see {\sffamily Part II Optimising Compilers} for details), but clearly this depends on the number of registers available in the target machine, and whether they are \emph{orthogonal} or if we have some special purpose registers.

    \textbf{Non-Example 2: Instruction Scheduling.} Once we've generated the linear list of instructions, we can simply blindly execute them in the order generated -- but this is by no means optimal! In {\sffamily Part IA Computer Architecture} you'll have learnt that loads take longer than other instructions, and that a common technique is to do \textit{load hoisting} --- schedule the load to run as early as possible so its value reaches a register before the next instruction tries to use it. How aggressively to do this depends on how long a load takes relative to other instructions, which of course is machine dependent.
