...
int f(int x) {
   ...
   // some code which never uses y
    ...
    \textbf{Non-Example 2: Instruction Scheduling.} Once we've generated the linear list of instructions, we can simply blindly execute them in the order generated -- but this is by no means optimal! In {\sffamily Part IB Computer Architecture} you'll have learnt that loads take longer than other instructions, and that a common technique is to do \textit{load hoisting} --- schedule the load to run as early as possible so its value reaches a register before the next instruction tries to use it. How aggressively to do this depends on how long a load takes relative to other instructions, which of course is machine dependent.
